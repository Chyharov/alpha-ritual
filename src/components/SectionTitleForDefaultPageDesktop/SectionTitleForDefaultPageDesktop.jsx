...
const SectionTitleForDefaultPageDesktop = ({ prevTitleLink, titleLink, backgroundImgDesktop, prevLink, title, description }) => {
  return (
    <section className={s.sectionTitleForDefaultPageDesktop}>
      <div className={s.bgimg}>
        ...
        <div className={s.linksContainer}>
          <Link to={prevLink} className='links'>{prevTitleLink}</Link>
          <img className={s.titleLine} src={line} alt="line" />
          <p className='linksIsActive'>{titleLink}</p>
        </div>

        <h2 className='title' style={{marginBottom: '40px'}}>{title}</h2>

        <p className={'description ' + s.descriptionForTitle}>{description}</p>
